...
def cmd_create(args: List[str]) -> Dict[str, Any]:
    ...
    if not args:
        ...
        print("Usage: mirrors create <event_key> --raw <path> [--summary <path>] [--lesson <path>]", file=sys.stderr)
        sys.exit(1)
...
def cmd_link(args: List[str]) -> Dict[str, Any]:
    ...
    if len(args) < 3:
        ...
        print("Usage: mirrors link <event_key> <raw_path> <summary_path> [vault_path]", file=sys.stderr)
        sys.exit(1)
