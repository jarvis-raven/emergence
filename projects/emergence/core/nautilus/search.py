...
def run_full_search(
    ...
    if chambers_filter:
        try:
            ...
            if verbose:
                print(
                    f"📂 Chamber filter ({chambers_filter}): {len(reranked)} results",
                    file=sys.stderr,
                )
        except Exception as e:
...
def _filter_by_chambers(results: List[Dict], allowed_chambers: Set[str]) -> List[Dict]:
    ...
    for r in results:
        ...
        try:
            # Look up chamber
            row = db.execute(
                "SELECT chamber FROM gravity WHERE path = ? LIMIT 1", (path,)
            ).fetchone()
...
def _apply_context_filter(
    results: List[Dict], context_tags: List[str], verbose: bool = False
) -> List[Dict]:
    """
    ...
    for r in results:
        ...
        try:
            row = db.execute(
                "SELECT tags, chamber, context_tags FROM gravity WHERE path = ? LIMIT 1", (path,)
            ).fetchone()
