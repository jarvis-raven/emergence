...
def cmd_status(args: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        corridor_count = len(list(config.get_corridors_dir().glob("*.md"))) if config.get_corridors_dir().exists() else 0
        vault_count = len(list(config.get_vaults_dir().glob("*.md"))) if config.get_vaults_dir().exists() else 0
        ...
        logger.info(f"Chamber status: {total} tracked, {corridor_count} corridors, {vault_count} vaults")
