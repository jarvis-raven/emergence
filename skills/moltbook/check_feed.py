...
def fetch_feed(api_key, limit=20):
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": "OpenClaw/Jarvis"}
    params = {"limit": limit}
...
def check_feed(limit=20, quarantine_threshold=2):
    ...
    return {"total_posts": total, "checked": len(results), "results": results}
...
def main():
    ...
    if args.json:
        ...
        for result in data["results"]:
            ...
            print(
                f"{status_icon} [{severity}:{score}] @{result['author']} (karma: {result['karma']})"
            )
