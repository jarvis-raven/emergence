...
def migrate_state_dir(state_dir: Path, dry_run: bool = False) -> dict:
    ...
    config = {"version": data.get("version", "1.1"), "drives": config_drives}
    ...
    print(
        f"   Config fields per drive: ~{stats['config_fields_moved'] // max(stats['drives_found'], 1)}"
    )
    print(
        f"   Runtime fields per drive: ~{stats['runtime_fields_moved'] // max(stats['drives_found'], 1)}"
    )
    ...
    print(
        f"   New drives.json: {config_size} bytes ({100 * config_size // original_size}% of original)"
    )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Re-migrate even if drives-state.json exists"
    )
