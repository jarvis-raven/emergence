...
def cmd_record_access(args):
    ...
    if not path:
        print("Usage: gravity.py record-access <path> [--lines START:END] [--query Q] [--score S]", file=sys.stderr)
        sys.exit(1)
...
def cmd_rerank(args):
    ...
    if not json_str:
        ...
        if not sys.stdin.isatty():
            ...
            print("Usage: gravity.py rerank --json '<results>' or pipe JSON via stdin", file=sys.stderr)
            sys.exit(1)
...
def cmd_supersede(args):
    ...
    if len(args) < 2:
        print("Usage: gravity.py supersede <old_path> <new_path> [--lines OLD_START:END NEW_START:END]", file=sys.stderr)
        sys.exit(1)
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in COMMANDS:
        print(f"Usage: gravity.py <command> [args]")
