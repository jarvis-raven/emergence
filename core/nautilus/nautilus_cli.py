...
from .config import get_workspace, get_config


def cmd_search(args):
    """Full Nautilus search pipeline."""
    if not args:
        print("Usage: emergence nautilus search <query> [--n 5] [--trapdoor] [--verbose]", file=sys.stderr)
        sys.exit(1)
    ...
    rerank_result = subprocess.run(
        [sys.executable, "-m", "core.nautilus.gravity", "rerank", "--json", json.dumps(raw_results)],
        capture_output=True, text=True, timeout=30
    )
    try:
    ...
    if not trapdoor and context_tags:
        ...
        for r in results:
            path = r.get('path', '')
            row = db.execute("SELECT tags, chamber FROM gravity WHERE path = ? LIMIT 1", (path,)).fetchone()
