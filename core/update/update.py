...
class UpdateManager:
    ...
    def check_room_running(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["pgrep", "-f", "room/server/index.js"], capture_output=True, timeout=5
            )
...
def main():
    ...
    parser.add_argument(
        "--check-only", action="store_true", help="Check for updates without installing"
    )
    parser.add_argument("--no-backup", action="store_true", help="Skip backup (not recommended)")
    parser.add_argument(
        "--force", action="store_true", help="Force update even if version check fails"
    )
