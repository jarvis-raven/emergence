...
def _fallback_similarity(new_name: str, new_desc: str,
                         existing_drives: dict, threshold: float) -> list[tuple[str, float, dict]]:
...
def generate_irreducibility_test(new_drive_name: str, new_drive_desc: str,
                                 similar_drives: list[tuple[str, float, dict]]) -> str:
...
def apply_irreducibility_decision(decision: str, new_drive_name: str,
                                  new_drive_desc: str, parent_drive_name: Optional[str],
                                  workspace: Path) -> dict:
    ...
    if decision == "DISTINCT":
        ...
        if new_drive_name in drives:
            return {
                "success": False, 
                "error": f"Drive {new_drive_name} already exists"
            }
        ...
        if not parent_drive_name or parent_drive_name not in drives:
            return {
                "success": False,
                "error": f"Parent drive {parent_drive_name} not found"
            }
...
def add_pending_review(workspace: Path, new_drive_name: str, new_drive_desc: str,
        ...
        for review in reviews:
            if review.get("new_drive") == specific_drive.upper():
                return generate_irreducibility_test(
                    ...
                    [(d["name"], d["similarity"], {}) 
                     for d in review.get("similar_drives", [])]
                )
