...
def check_first_light_completion(workspace: Path, auto_complete: bool = True) -> dict:
    ...
    gates_met = (
        gate_status["sessions_met"] and gate_status["days_met"] and gate_status["drives_met"]
    )
    ...
    if gates_met and auto_complete:
        ...
        if not gate_status["sessions_met"]:
            missing.append(
                f"sessions ({fl_data.get('session_count', 0)}/{fl_data.get('gates', {}).get('min_sessions', 10)})"
            )
        if not gate_status["days_met"]:
            missing.append("days elapsed")
        if not gate_status["drives_met"]:
            missing.append(
                f"drives ({len(fl_data.get('discovered_drives', []))}/{fl_data.get('gates', {}).get('min_discovered_drives', 3)})"
            )
    ...
    return {
        ...
        "progress": {
            "sessions": {
                ...
                "percent": min(
                    100, int((fl_data.get("session_count", 0) / gates["min_sessions"]) * 100)
                ),
            },
            "days": {
        ...
        "gate_status": gate_status,
        "can_complete": (
            fl_data["status"] == "active"
            and gate_status["sessions_met"]
            and gate_status["days_met"]
            and gate_status["drives_met"]
        ),
...
def format_status_display(status: dict) -> str:
    ...
    lines.append(
        f"  Sessions: {progress_bar(sessions['percent'])} {sessions['current']}/{sessions['required']}"
    )
    lines.append(
        f"  Days:     {progress_bar(days['percent'])} {days['current']}/{days['required']}"
    )
    lines.append(
        f"  Drives:   {progress_bar(drives['percent'])} {drives['current']}/{drives['required']}"
    )
    lines.append("")
    ...
    lines.append("Gates:")
    lines.append(
        f"  {'✓' if gate_status['sessions_met'] else '○'} Sessions: {sessions['current']}/{sessions['required']}"
    )
    lines.append(
        f"  {'✓' if gate_status['days_met'] else '○'} Days: {days['current']}/{days['required']}"
    )
    lines.append(
        f"  {'✓' if gate_status['drives_met'] else '○'} Drives: {drives['current']}/{drives['required']}"
    )
...
def manual_complete_first_light(workspace: Path, force: bool = False) -> dict:
    ...
    if result["success"]:
        return {"success": True, "forced": force, "message": result["message"]}
    else:
        return result
...
def check_grandfather_eligibility(workspace: Path) -> dict:
    ...
    return {
        ...
        "gates": {
            "sessions": {
                "required": gates.get("min_sessions", 10),
                "found": session_count,
                "met": sessions_met,
            },
            "days": {
                "required": gates.get("min_days_elapsed", 7),
                "found": unique_days,
                "met": days_met,
            },
            "drives": {
                "required": gates.get("min_discovered_drives", 3),
                "found": len(drive_names),
                "met": drives_met,
            },
        },
    }
...
def grandfather_first_light(workspace: Path) -> dict:
    ...
    if not eligibility["eligible"]:
        ...
        if not gates["sessions"]["met"]:
            missing.append(
                f"sessions ({gates['sessions']['found']}/{gates['sessions']['required']})"
            )
    ...
    fl_data["gate_status"] = {
        ...
        "days_met": True,
        "drives_met": True,
        "over_soft_limit": len(evidence["drive_names"])
        > fl_data.get("gates", {}).get("max_drives_soft_limit", 8),
    }
