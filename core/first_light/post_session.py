...
def check_similarity_and_queue_review(workspace: Path, drive_meta: dict) -> bool:
    ...
    similar = find_similar_drives(
        drive_meta["name"], drive_meta["description"], existing_drives, workspace
    )
...
def analyze_session(
    workspace: Path, session_file: Path, auto_activate: bool = True, check_similarity: bool = True
):
...
def analyze_recent_sessions(
    workspace: Path, limit: int = 5, auto_activate: bool = True, check_similarity: bool = True
):
    ...
    for session_file in fl_sessions:
        analyze_session(
            workspace, session_file, auto_activate=auto_activate, check_similarity=check_similarity
        )
