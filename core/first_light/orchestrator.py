...
def spawn_via_cli(prompt: str, config: dict, session_num: int) -> bool:
    ...
    cmd = [
        ...
        "--delete-after-run",
        "--name",
        f"first-light-session-{session_num}",
        "--no-deliver",
    ]
...
def run_first_light_tick(config: dict, dry_run: bool = False) -> dict:
    ...
    state["next_run_time"] = calculate_next_run_time(
        datetime.now(timezone.utc).isoformat(), frequency_hours
    )
...
def get_status(config: dict) -> dict:
    ...
    size = int(get_config_value(config, "size", 3))
    model = (
        get_config_value(config, "model", None) or config.get("agent", {}).get("model") or "default"
    )
...
def main():
    ...
    analyze_parser.add_argument(
        "--limit", type=int, default=5, help="Number of recent sessions to analyze (default: 5)"
    )
    analyze_parser.add_argument(
        "--no-activate", action="store_true", help="Register but don't activate drives"
    )

    # complete command
    complete_parser = subparsers.add_parser(
        "complete", help="Complete First Light phase (graduation)"
    )
    complete_parser.add_argument(
        "--force", action="store_true", help="Complete even if gates not met"
    )
    complete_parser.add_argument(
        "--grandfather",
        action="store_true",
        help="Grandfather pre-v0.2.0 agents with historical sessions",
    )

    # grandfather command (alias for complete --grandfather)
    subparsers.add_parser(
        "grandfather", help="Complete First Light for pre-v0.2.0 agents with historical sessions"
    )
