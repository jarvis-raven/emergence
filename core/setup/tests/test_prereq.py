...
class TestOllamaInstalledCheck(unittest.TestCase):
    ...
    def test_ollama_installed(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout="ollama version 0.1.24\n", stderr="")
...
class TestOllamaModelCheck(unittest.TestCase):
    ...
    def test_empty_model_list(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout="NAME\tID\tSIZE\tMODIFIED", stderr="")
    ...
    def test_ollama_list_fails(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=1, stdout="", stderr="Error: could not connect to server"
        )
...
class TestInstallationHelpers(unittest.TestCase):
    ...
    def test_pull_model_success(self, mock_run):
        ...
        self.assertTrue(result)
        mock_run.assert_called_once_with(
            ["ollama", "pull", "nomic-embed-text"], capture_output=True, text=True, timeout=600
        )
...
class TestJsonOutput(unittest.TestCase):
    ...
    def test_json_structure_all_pass(
        self, mock_platform, mock_model, mock_ollama, mock_gateway, mock_node, mock_python
    ):
    ...
    @patch("prereq.check_openclaw_gateway")
    @patch("prereq.check_ollama_installed")
    def test_json_hard_deps_fail(self, mock_ollama, mock_gateway, mock_node, mock_python):
...
class TestIntegration(unittest.TestCase):
    ...
    def test_soft_deps_missing(
        self, mock_pull, mock_install, mock_model, mock_ollama, mock_gateway, mock_node, mock_python
    ):
