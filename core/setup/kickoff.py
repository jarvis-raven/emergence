...
def _schedule_drives_tick(emergence_dir: Path, tick_interval_min: int = 15) -> bool:
    ...
    # The cron command
    cron_cmd = f"cd {emergence_dir} && python3 -m core.drives tick >> /tmp/emergence-drives.log 2>&1"
...
def kickoff_first_light(config: dict, workspace: Path) -> bool:
    ...
    if emergence_dir:
        ...
        if fl_tick_ok:
            print(f"  ✓ First Light tick scheduled (every {freq_hours:.0f}h, ~{sessions_per_day} sessions/day)")
        else:
    ...
    try:
        from ..first_light.orchestrator import (
            load_config, load_first_light_state, 
            schedule_exploration_session, save_first_light_state,
            calculate_next_run_time, parse_frequency, get_config_value
        )
...
def main():
    ...
    parser.add_argument(
        "--answers",
        type=Path,
        help="JSON file with wizard answers"
    )
    parser.add_argument(
        "--plan",
        type=Path,
        help="JSON file with placement plan"
    )
