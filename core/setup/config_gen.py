...
def validate_config(config: dict) -> list[str]:
    ...
    if not isinstance(port, int):
        ...
        errors.append(f"room.port must be between 1024 and 65535 (or 0 to disable), got {port}")
  ...
  %(prog)s --validate config.json   # Validate existing config
    ...
    if args.validate:
        try:
            ...
            errors = validate_config(config)
            if errors:
                print(f"❌ Config validation failed for: {args.validate}")
                for err in errors:
                    print(f"   • {err}")
                sys.exit(1)
            else:
                print(f"✅ Config is valid: {args.validate}")
                sys.exit(0)
        except FileNotFoundError:
            print(f"❌ Config file not found: {args.validate}", file=sys.stderr)
            sys.exit(1)
