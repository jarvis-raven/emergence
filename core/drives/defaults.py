    ...
    for name, override in overrides.items():
        ...
        for field, value in override.items():
            if field not in ALLOWED_OVERRIDE_FIELDS:
                errors.append(f"Cannot override '{field}' for {name} — field is protected")
                continue
            ...
            if field in ("threshold", "rate_per_hour"):
                try:
                    ...
                    if float_value < 0:
                        errors.append(f"Invalid value for {name}.{field}: must be non-negative")
                    elif field == "threshold" and float_value == 0:
                        errors.append(f"Warning: {name}.threshold=0 means drive never triggers")
                except (ValueError, TypeError):
                    errors.append(f"Invalid value for {name}.{field}: must be a number")
