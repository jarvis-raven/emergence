...
from .runtime_state import extract_runtime_state
from .engine import (
    tick_all_drives,
    check_thresholds,
    satisfy_drive,
...
from .history import filter_log_entries
...
def _print_drive_line(name: str, drive: dict, triggered: set, cooldown_info: dict) -> None:
    ...
    if status == "triggered":
        ...
        if remaining > 0:
            ...
            status_text = (
                f"{COLOR_BUDGET_HIGH}Over threshold{COLOR_RESET} ({band_color}{band}{COLOR_RESET})"
            )
...
def cmd_satisfy(args) -> int:
    ...
    if auto_scaled:
        ...
        thresholds = get_drive_thresholds(drive, config.get("drives", {}).get("thresholds"))
        band_name, _, reduction_ratio = calculate_satisfaction_depth(
            old_pressure, threshold, thresholds
        )
    ...
    if auto_scaled:
        ...
        print(
            f"✓ {drive_name} satisfied ({old_pressure:.1f} → {new_pressure:.1f}) [band: {band_display}]"
        )
    ...
    if new_pressure > 0.0:
        if auto_scaled:
            print(
                f"  For more relief, use explicit depth: 'drives satisfy {drive_name.lower()} deep' or 'full'"
            )
...
def cmd_complete(args) -> int:
    ...
    if not args.name:
        print(
            "✗ Usage: drives complete <drive_name> [--session-key KEY] [--status STATUS]",
            file=sys.stderr,
        )
        return EXIT_USAGE
    ...
    if not success:
        print(
            f"✗ Failed to update session status (session not found or already completed)",
            file=sys.stderr,
        )
        return EXIT_ERROR
...
def cmd_tick(args) -> int:
    ...
    for name, (old, new) in changes.items():
        ...
        if verbose:
            ...
            print(
                f"  {name}: {old:.1f}/{threshold:.0f} ({old_pct}%) → {new:.1f}/{threshold:.0f} ({new_pct}%)"
            )
...
def cmd_show(args) -> int:
    ...
    print()
    print(
        f"Pressure: {status['pressure']:.1f} / Threshold: {status['threshold']:.0f} ({int(status['percentage'])}%)"
    )
    ...
    if dry_run:
        ...
        for impact in impacts:
            ...
            if drive_name in drives:
                ...
                print(
                    f"  {direction} {drive_name}: {old_pressure:.1f} → {new_pressure:.1f} ({delta:+.0f}) — {reason}"
                )
            else:
    ...
    if action == "status":
        ...
        if platform_name in ("macos", "linux"):
            ...
            if install_result.get("success"):
                ...
                print()
                print(
                    f"  Next tick in ~{config.get('drives', {}).get('tick_interval', 900) // 60} minutes"
                )
        ...
        if result["success"]:
            ...
            print(
                f"  Logs: {config.get('paths', {}).get('workspace', '.')}/.emergence/logs/daemon.log"
            )
        else:
        ...
        if stop_result.get("was_running") and not stop_result["success"]:
            print(
                f"✗ Failed to stop daemon: {', '.join(stop_result.get('errors', []))}",
                file=sys.stderr,
            )
            return EXIT_ERROR
        ...
        if result["success"]:
            ...
            print(
                f"✗ Failed to start daemon: {', '.join(result.get('errors', []))}", file=sys.stderr
            )
            return EXIT_ERROR
        ...
        log_path = (
            Path(config.get("paths", {}).get("workspace", "."))
            / ".emergence"
            / "logs"
            / "daemon.log"
        )
        ...
        print(
            "  Valid actions: start, stop, restart, status, logs, install, uninstall",
            file=sys.stderr,
        )
        return EXIT_USAGE
...
def cmd_review(args) -> int:
    ...
    if decision and drive_name:
        ...
        result = apply_irreducibility_decision(
            decision, review["new_drive"], review["new_drive_description"], parent_drive, workspace
        )
...
def cmd_dashboard(args) -> int:
    ...
    if triggered_drives:
        ...
        print(
            f"    emergence drives satisfy {first_triggered.lower()} [shallow|moderate|deep|full]"
        )
    ...
    if emergency_warnings:
        ...
        for d in emergency_warnings:
            ...
            if d["ratio"] >= 2.0:
                print(
                    f"    {COLOR_BUDGET_HIGH}🚨 {d['name']} at {pct}% — EMERGENCY AUTO-SPAWN ACTIVE{COLOR_RESET}"
                )
            else:
                print(
                    f"    {COLOR_WARNING}⚠ {d['name']} at {pct}% — approaching emergency threshold (200%){COLOR_RESET}"
                )
...
def cmd_migrate(args) -> int:
    ...
    if not dry_run:
        ...
        total_events = sum(
            len(d.get("satisfaction_events", [])) for d in state.get("drives", {}).values()
        )
        if total_events > 0:
...
""",
    ...
    status_parser = subparsers.add_parser(
        "status", help="Show drive status with pressure bars (default)", aliases=["st"]
    )
    status_parser.add_argument("--json", action="store_true", help="Output as JSON")
    status_parser.add_argument("--category", help="Filter by category")
    status_parser.add_argument(
        "--show-latent", action="store_true", help="Show latent/consolidated drives"
    )

    # dashboard command
    dashboard_parser = subparsers.add_parser(
        "dashboard", help="Show all drives grouped by urgency", aliases=["dash"]
    )
    dashboard_parser.add_argument(
        "--show-all", action="store_true", help="Include low-pressure drives (<30%)"
    )

    # satisfy command
    satisfy_parser = subparsers.add_parser(
        "satisfy", help="Reduce pressure after addressing a drive", aliases=["sat"]
    )
    satisfy_parser.add_argument("name", nargs="?", help="Name of drive to satisfy (fuzzy matched)")
    satisfy_parser.add_argument(
        "depth", nargs="?", help="Satisfaction depth: shallow/moderate/deep/full (or s/m/d/f)"
    )
    satisfy_parser.add_argument("--reason", help="Reason for satisfaction (logged)")

    # complete command - signal session completion
    complete_parser = subparsers.add_parser(
        "complete", help="Signal that a drive session has completed (triggers satisfaction)"
    )
    complete_parser.add_argument(
        "name", nargs="?", help="Name of drive to mark complete (fuzzy matched)"
    )
    complete_parser.add_argument(
        "--session-key", help="OpenClaw session key (optional, for precise matching)"
    )
    complete_parser.add_argument(
        "--status",
        default="completed",
    ...
    # bump command
    bump_parser = subparsers.add_parser("bump", help="Manually increase pressure")
    bump_parser.add_argument("name", nargs="?", help="Name of drive to bump (fuzzy matched)")
    bump_parser.add_argument("amount", nargs="?", help="Amount to add (default: 2 hours worth)")
    bump_parser.add_argument("--reason", help="Reason for bump (logged)")

    # reset command
    reset_parser = subparsers.add_parser("reset", help="Zero all drive pressures")
    reset_parser.add_argument("--force", action="store_true", help="Skip confirmation prompt")

    # log command
    log_parser = subparsers.add_parser(
        "log", help="Show trigger/satisfaction history", aliases=["history"]
    )
    log_parser.add_argument("n", nargs="?", help="Number of entries to show (default: 20)")
    log_parser.add_argument("--drive", help="Filter to specific drive")
    log_parser.add_argument("--since", help="Show entries since time (e.g., '2 hours ago')")

    # tick command
    tick_parser = subparsers.add_parser("tick", help="Update pressures and check for triggers")
    tick_parser.add_argument(
        "--dry-run", action="store_true", help="Show what would happen without making changes"
    )
    tick_parser.add_argument(
        "--verbose", action="store_true", help="Show detailed per-drive calculations"
    )

    # list command
    list_parser = subparsers.add_parser(
        "list", help="List all drives with metadata", aliases=["ls"]
    )
    ...
    show_parser = subparsers.add_parser(
        "show", help="Show detailed info for a drive", aliases=["info"]
    )
    show_parser.add_argument("name", nargs="?", help="Name of drive to show (fuzzy matched)")

    # ingest command
    ingest_parser = subparsers.add_parser("ingest", help="Analyze content for drive impacts")
    ingest_parser.add_argument(
        "file", nargs="?", help="Path to file to ingest (optional if --recent)"
    )
    ingest_parser.add_argument("--recent", action="store_true", help="Ingest today's memory files")
    ingest_parser.add_argument(
    ...
    daemon_parser.add_argument(
        "--foreground", "-f", action="store_true", help="Run daemon in foreground (don't detach)"
    )
    daemon_parser.add_argument(
        "--platform", choices=["macos", "linux", "cron"], help="Platform for install/uninstall"
    )
    daemon_parser.add_argument(
        "--n", type=int, default=50, help="Number of log lines to show (default: 50)"
    )

    # review command
    review_parser = subparsers.add_parser(
        "review", help="Review pending drive consolidation decisions"
    )
    review_parser.add_argument(
        "name", nargs="?", help="Drive name to review (optional, lists all if omitted)"
    )
    review_parser.add_argument(
        "--decide", choices=["DISTINCT", "ASPECT"], help="Apply irreducibility decision"
    )
    review_parser.add_argument("--parent", help="Parent drive name (if ASPECT decision)")

    # activate command
    activate_parser = subparsers.add_parser("activate", help="Activate a latent/consolidated drive")
    activate_parser.add_argument("name", nargs="?", help="Name of latent drive to activate")

    # aspects command
    aspects_parser = subparsers.add_parser("aspects", help="Manage drive aspects")
    aspects_parser.add_argument("name", nargs="?", help="Drive name to manage aspects for")
    aspects_parser.add_argument(
        "action", nargs="?", choices=["list", "add", "remove"], help="Aspect management action"
    )
    aspects_parser.add_argument("aspect_name", nargs="?", help="Name of aspect to add/remove")

    # migrate command
    migrate_parser = subparsers.add_parser(
        "migrate", help="Migrate breadcrumbs to trigger-log.jsonl (Phase 3)"
    )
    ...
    # help command
    help_parser = subparsers.add_parser("help", help="Show detailed help for topics")
    help_parser.add_argument(
        "topic", nargs="?", help="Help topic (satisfy, tick, bump, daemon, review, etc.)"
    )
