...
from .models import Drive, DriveState, SATISFACTION_DEPTHS
...
def tick_all_drives(state: DriveState, config: dict) -> dict:
...
def check_thresholds(
    state: DriveState, config: dict, respect_quiet_hours: bool = True
) -> list[str]:
...
def satisfy_drive(state: DriveState, drive_name: str, depth: str = "moderate") -> dict:
...
def mark_drive_triggered(state: DriveState, drive_name: str) -> dict:
...
def bump_drive(state: DriveState, drive_name: str, amount: Optional[float] = None) -> dict:
...
def get_drive_status(
    state: DriveState, drive_name: str, config: Optional[dict] = None
) -> Optional[dict]:
...
def cleanup_stale_triggers(state: DriveState, config: dict, max_age_minutes: int = 60) -> list[str]:
