...
def get_recent_memory_files(
    memory_dir: Path, lookback_days: int = 7, reference_date: Optional[datetime] = None
) -> list[Path]:
...
def calculate_tf_idf(
    file_phrases: dict[str, list[str]], max_concepts: int = 50
    ...
    for phrase, doc_count in doc_freq.items():
        ...
        sources = sorted(phrase_counts[phrase].items(), key=lambda x: x[1], reverse=True)[:3]
...
class ConceptExtractor:
    ...
    def extract(self) -> list[dict]:
        ...
        files = get_recent_memory_files(self.memory_dir, self.lookback_days, self.reference_date)
