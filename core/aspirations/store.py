...
def _project_count_for_aspiration(data: AspirationsData, aspiration_id: str) -> int:
    ...
    return sum(1 for p in data.get("projects", []) if p.get("aspirationId") == aspiration_id)
...
def remove_aspiration(
    data: AspirationsData, aspiration_id: str, force: bool = False
) -> tuple[bool, str]:
