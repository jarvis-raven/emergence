...
export function PressureBar({
  ...
  const { name, pressure, threshold, percentage, band, bandIcon, thresholds, bandColors } =
    enrichedDrive;
    ...
    >
      ...
      {showDetails && (
        <div className="text-[8px] text-textMuted mt-1 text-center space-y-0.5">
          <div>
            {pressure?.toFixed?.(1) || pressure}/{threshold}
          </div>
          <div className={`${bandColors.text} font-semibold`}>{band}</div>
        </div>
      )}
    </div>
  );
